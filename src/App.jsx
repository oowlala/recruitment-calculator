      ...
      <div className="relative z-10 max-w-6xl mx-auto p-4 md:p-8 space-y-6">
        <header className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
          ...
          <div className="mt-4 md:mt-0 flex gap-3">
             ...
             <button className="px-8 py-2.5 bg-[#4f46e5] text-white rounded-xl shadow-lg font-bold text-sm transition-all hover:scale-[1.02]">Save to Sheet</button>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-7 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                   ...
                   <div className="relative">
                    <select name="origCurrency" value={inputs.origCurrency} onChange={handleInputChange} disabled={inputs.isJapanPartner} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-700 outline-none">
                ...
                <div className="space-y-2">
                   ...
                   <div className="relative">
                    <select name="multiplier" value={inputs.isJapanPartner ? 1 : inputs.multiplier} onChange={handleInputChange} disabled={inputs.isJapanPartner} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-700 outline-none">
                ...
                <div className="md:col-span-2 space-y-2">
                   ...
                   <input type="text" name="monthlySalary" value={inputs.monthlySalary} onChange={handleInputChange} placeholder="Required" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-xl text-slate-700 focus:bg-white outline-none transition-all" />
                </div>
                <div className="space-y-2">
                   <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Additional</label>
                ...
                <div className="space-y-2">
                   <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Signup Bonus</label>
            ...
            <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
              <h2 className="text-sm font-black text-pink-500 uppercase tracking-widest flex items-center gap-2"><TrendingUp size={18} /> Fee Structure</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="bg-[#1e293b] p-8 rounded-[2.5rem] shadow-2xl text-white border border-slate-700 relative overflow-hidden">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end border-b border-slate-700/50 pb-4"><span className="text-slate-400 text-xs font-bold uppercase">Billable Salary</span><span className="text-xl font-black tracking-tight">{formatMoney(totalSalary)}</span></div>
                ...
                {inputs.isJapanPartner && (
                  <div className="flex justify-between items-end border-b border-pink-900/40 pb-4">
                    <span className="text-pink-400 text-[10px] font-black uppercase tracking-widest">Partner Deduction</span>
                    <span className="text-lg font-black text-pink-400">-{formatMoney(deduction)}</span>
                  </div>
                )}
                <div className="pt-6 relative">
                  ...
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 to-pink-300 drop-shadow-sm">{formatMoney(finalFee)}</span>
                  </div>
                </div>
              </div>
            </div>

            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative space-y-6">
               <h2 className="text-sm font-black text-indigo-500 uppercase tracking-widest flex items-center gap-2"><ArrowRightLeft size={18} /> Live Conversions</h2>
                {[1, 2, 3].map(idx => (
                  <div key={idx} className="flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50 transition-all group">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-2xl font-black text-slate-800 tracking-tighter">
                        {formatNumber(finalFee * (rates[inputs[`target${idx}`]] || 0))}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="mt-12 bg-white rounded-[3rem] p-10 border border-slate-100 shadow-xl relative overflow-hidden group">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-8">
            <div className="flex items-center gap-3">
              <span className="text-6xl font-black text-[#1e293b] tracking-tighter leading-none">RAS</span>
              <div className="w-1.5 h-12 bg-pink-500 rounded-full"></div>
              <div className="flex flex-col font-black text-slate-400 uppercase text-xs tracking-widest"><span>Currency</span><span>Converter</span></div>
            </div>
            <div className="text-right"><span className="text-[10px] font-black text-slate-300 uppercase mb-1 block tracking-widest">DATE</span><div className="border-b-2 border-slate-100 pb-1 px-4 text-sm font-black text-slate-600">{forexLastUpdate || 'LIVE MARKET RATE'}</div></div>
          </div>
          <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-[1.5rem] border border-slate-100 shadow-inner mb-12">
            <div className="relative">
              <select name="baseCurrency" value={forex.baseCurrency} onChange={(e) => setForex({...forex, baseCurrency: e.target.value})} className="pl-4 pr-10 py-2 bg-white border-none rounded-xl font-black text-xl text-indigo-600 shadow-sm outline-none">
                {CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <ChevronDown size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-indigo-300" />
            </div>
            <input type="text" value={forex.amount} onChange={(e) => setForex({...forex, amount: formatInputNumber(e.target.value)})} className="bg-transparent text-4xl font-black text-right w-full outline-none pr-4" placeholder="0" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            {[forex.target1, forex.target2, forex.target3].map((curr, i) => {
              const rate = forexRates[curr] || 0;
              return (
                <div key={i} className="flex flex-col border-b border-dashed border-slate-200 pb-4">
                  <div className="flex justify-between items-center mb-3">
                    <span className="font-black text-slate-500 uppercase tracking-widest text-sm">{curr}</span>
                    <span className="text-[10px] font-bold text-slate-300">FX: {rate?.toFixed(4)}</span>
                  </div>
                  <div className="text-3xl font-black text-slate-800 tracking-tighter tabular-nums">{formatNumber(parseInputNumber(forex.amount) * rate)}</div>
                </div>
              );
            })}
          </div>
          <div className="flex flex-col items-center">
            <div className="font-serif text-5xl text-slate-700/80 italic transform -rotate-1 select-none mb-1">Elon Musk</div>
